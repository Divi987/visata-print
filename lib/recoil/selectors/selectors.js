...
import { categoryFilterByParentIdState, categoryState, categoryStateSlug } from "../atoms/states";
...
export const categoryFilterByParentSelector = selector({
    ...
    get: async ({ get }) => {
        try {
            ...
            const response = await fetch(`/api/categories?parentId=${selectedCategoryId}`);
            if (!response.ok) throw new Error('Failed to fetch category');
            return await response.json();
            } catch (error) {
            console.error(error);
